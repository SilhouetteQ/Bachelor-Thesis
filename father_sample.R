...
#   r2[i-1] <- m[[3]]
# }
# s <- data.frame(vari[-1], var_num, sam_size, r2)
# colnames(s) <- c("var_num", "sam_size", "r2")
# write.table(s, file = "out_sample.csv",row.names = FALSE)
# read.csv("out_sample.csv",sep = "")

  
  
  # r2csv <- read.csv("out_sample.csv",sep = "")
  # plot(r2csv[,1],r2csv[,3],  lwd = 2, cex = .2, ann = F)
  # # text(num,r2csv[,2], labels = paste(1:length(select_var),".",select_var))
  # lines(r2csv[,1],r2csv[,3], col = "red", lty = 1, lwd = 2)
  # par(new = TRUE)
  # plot(r2csv[,1],r2csv[,2],  lwd = 2, cex = .2,  ann = F, xaxt = "n", yaxt ="n")
  # lines(r2csv[,1],r2csv[,2], col = "blue", lty = 1, lwd = 2)
