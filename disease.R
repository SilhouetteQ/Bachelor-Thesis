...
yinformat <- subset(informat, phenotype == "4079_irnt", select = c(phenotype, mean, var))
xinformat <-  subset(informat, phenotype != "4079_irnt", select = c(phenotype, mean, var))

##### obtain yxcorrelation and xxcorrelation
vec <- which(colnames(LD_phenomat)== "4079_irnt")
...
### dim(whi) = 57160 1755
vari <- c("4079_irnt", res$selected.markers)
